...
Bars.findAllByEventId = (req, res, next) => {
  ...
  db.manyOrNone('SELECT * FROM bars WHERE eventId=$1', [eventId] // use the id here
  ).then(data => {
    //console.log(data);
...
Bars.findAllBarData = (req, res, next) => {
  ...
  axios.all(barDataCalls).then(response => {
    ...
    response.forEach((response) => {
      const name = response.data.response.venue.name,
        ...
        daysOpen = response.data.response.venue.hours
          ? response.data.response.venue.hours.timeframes[0].days
          : 'Not Found',
        hoursOpen = response.data.response.venue.hours
          ? response.data.response.venue.hours.timeframes[0].open[0].renderedTime
          : 'Not Found',
        hoursUntilClosed = response.data.response.venue.hours
          ? response.data.response.venue.hours.status
          : 'Not Found',
        isOpen = response.data.response.venue.hours
          ? response.data.response.venue.hours.isOpen
          : 'Not Found',
...
Bars.findOneBarById = (req, res, next) => {
  ...
  db.one(`SELECT * FROM bars WHERE eventId = $1 AND id = $2`, [eventId, id]).then(data => {
    res.locals.bar = data;
    next();
...
Bars.findOneBarData = (req, res, next) => {
  ...
  axios.get(`https://api.foursquare.com/v2/venues/${barId}?client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&v=${DATE}`).then(response => {
...
Bars.searchNearbyBars = (req, res, next) => {
  ...
  axios.get(`https://api.foursquare.com/v2/venues/search?ll=${lat},${long}&categoryId=4d4b7105d754a06376d81259&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&v=${DATE}&limit=10&radius=1500`).then(response => {
...
Bars.searchBars = (req, res, next) => {
  ...
  const lat = res.locals.latLong.lat;
  const long = res.locals.latLong.lng;
  axios.get(`https://api.foursquare.com/v2/venues/search?ll=${lat},${long}&query=${searchTerm}&categoryId=4d4b7105d754a06376d81259&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&v=${DATE}&limit=10`).then(response => {
    const fiveResults = response.data.response.venues;
    res.locals.fiveResults = fiveResults.map(result => {
      return {barId: result.id, name: result.name, lat: result.location.lat, long: result.location.lng}
    });
